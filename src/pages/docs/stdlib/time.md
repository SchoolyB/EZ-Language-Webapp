...
do main() {
    // Current date/time
    mut now int = time.now()
    println("Current time:", time.format(now, "YYYY-MM-DD HH:mm:ss"))

    // Calculate age
    mut birthday int = time.make(1990, 6, 15)
    mut age_seconds int = time.diff(now, birthday)
    mut age_years int = age_seconds / (365 * 24 * 60 * 60)
    println("Age:", age_years, "years")

    // Future date
    mut next_week int = time.add_days(now, 7)
    println("Next week:", time.format(next_week, "MMM DD, YYYY"))

    // Countdown timer
    println("Starting 3 second countdown...")
